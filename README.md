...
초기에는 Claude API가 애매한 링크를 `분류 불가`로 처리하는 경우가 많았습니다.  
이를 개선하기 위해 다음과 같은 방식을 적용했습니다.

- 카테고리 설명을 함께 제공해 모델이 분류 기준을 이해하도록 함
- 최근 분류 이력을 few-shot 예시로 제공
- `분류 불가`보다 가장 가까운 카테고리를 선택하도록 지시문 수정
- 자주 사용된 태그 위주로 입력을 줄여 비용과 처리량 최적화
- 모델 응답을 JSON 형태로 파싱해 앱에서 안정적으로 활용
